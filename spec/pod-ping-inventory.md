...
Status tags: **live** (in production), **step-2** (part of the launcher-convergence work, not built yet), **changed** (reworked by the nudge redesign, step 1).
...
There are also two role resolvers that must agree but currently do not: `slot_role_name_for_event_time` (`pod_signals.py`, documented as the single source of truth, keyed on the buckets above) and `slot_for_event_time` (`pod_schedule.py`, keyed on `WEEKLY_SLOTS` which only knows Wed-Late / Thu-Early / Sat-15:00). The T-1 rally still reads the second one, so weekend launcher pods off the `WEEKLY_SLOTS` grid resolve to no rally ping. Unifying on `slot_role_name_for_event_time` is step-2 cleanup.
...
|------|------|---------|-------|-------|------|--------|
| **T-1h rally** | 1h before a scheduled/launcher pod, only when close (needs ≤ `close_gap` to reach the aim) | slot role for the pod's time | ping | `pod_underfill_check_hours` (3,2,1), `pod_underfill_ping_hours` (1), `pod_underfill_ping_close_gap` (2), `pod_draft_target_players` (8) | `bot/tasks/pod_underfill.py` `_nudge_ping_role` | changed |
...
| **Launcher slot nudge** | count-triggered when a lazy launcher slot hits one short of the fire floor, any hour | bucket (slot) role | ping | `pod_signal_fire_threshold` (6), `POLL_NUDGE_QUIET_MINUTES` (30), `pod_nudge_in_chat` (true) | `bot/tasks/pod_daily_poll.py` `_maybe_nudge_slot` | to be replaced in step-2 by the T-3/T-1 schedule |
| **Launcher creation announcement** | when a launcher slot fires within 3h of its time; numberless, so it can't go stale | slot role | ping | within-3h window (new knob) | `bot/tasks/pod_daily_poll.py` `_launch_slot` | step-2 |
...
| **Queue nudge** | once, when an on-demand `/draft` queue is one short of firing, after a quiet window; silent if it fills fast | Pod Draft Queue (opt-in holders) | ping | `pod_signal_fire_threshold` (6), `QUEUE_NUDGE_QUIET_MINUTES` (30) | `bot/commands/pod_queue.py` `_maybe_nudge` | copy cleanup pending (step-3) |
...
- `pod_nudge_in_chat` = true — launcher slot nudge lands in pod-draft-chat with a jump link, versus in place under the launcher.
...
- `POLL_NUDGE_QUIET_MINUTES` = 30 (`pod_daily_poll.py`), `QUEUE_NUDGE_QUIET_MINUTES` = 30 (`pod_queue.py`) — quiet windows before a count-based nudge repeats.
...
- **Two role resolvers disagree** (`slot_role_name_for_event_time`, bucket-based, vs `slot_for_event_time`, `WEEKLY_SLOTS`-based). The T-1 rally still reads the second, which only knows the Sat-15:00 weekly slot; it now correctly resolves that one to Weekend Early Pod, but weekend evening / Sunday launcher pods resolve to no rally ping at all. Unify on `slot_role_name_for_event_time` in step-2 so the rally covers every real slot.
...
- **Launcher slot nudge is still count-triggered** and can fire at any hour; step-2 moves it onto the T-3/T-1 schedule so it obeys "never ping far from the draft time."
