...
def test_auto_grant_maps_timed_weekday_slots_to_their_roles():
    ...
    assert auto_grant_spec_for_event(thursday).name == EURO_POD_DRAFTERS_ROLE_NAME
    assert auto_grant_spec_for_event(saturday).name == WEEKEND_POD_DRAFTERS_ROLE_NAME
    assert auto_grant_spec_for_event(wednesday) is None
...
def test_blurb_with_time_appends_a_localized_timestamp_for_slot_roles():
    blurb = blurb_with_time(_spec_named(EURO_POD_DRAFTERS_ROLE_NAME))
